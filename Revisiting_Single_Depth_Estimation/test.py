...
def main():
    #print("=> loading checkpoint 14")
    model = define_model(is_resnet=False, is_densenet=False, is_senet=True)
    model = torch.nn.DataParallel(model).cuda()
    model.load_state_dict(torch.load('./pretrained_model/model_senet'))

  
    #checkpoint = torch.load('./pretrained_model/checkpoint14.pth.tar')
...
def remove_module_from_checkpoint_state_dict(state_dict):
    ...
    for k, v in state_dict.items():
        #print("=> loaded checkpoint '{}' (epoch {})".format(k, v))
        name = k  # remove `module.`
        print(name)
        #new_state_dict["module."+name] = v
        #print("=> loaded checkpoint '{}' (epoch {})".format("module."+name, v))
